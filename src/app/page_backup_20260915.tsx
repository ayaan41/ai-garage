...
export default function HomeBookingForm() {
  ...
  const handleBooking = async () => {
    ...
    setLoading(false); alert(`✅ Booking Created! Ref: ${bookingRef}\nTrack: /track/${bookingRef}`); window.location.href = `/track/${bookingRef}`;
  };
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4"><div className="max-w-md w-full bg-zinc-900 rounded-3xl p-8 border border-zinc-800">
      <h1 className="text-3xl font-bold">haji auto center</h1><p className="text-zinc-400 text-sm">G40 1EU, Glasgow - Book Your Car Service</p><p className="text-yellow-400 text-xs mt-2">Instant Booking • SMS Tracking • Invoice & Payment</p>
      <div className="mt-8 space-y-4">
        <div><label className="text-xs text-zinc-500">Your Name</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Ahmed" className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm" /></div>
        <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-zinc-500">Car Reg *</label><input value={carReg} onChange={e=>setCarReg(e.target.value)} placeholder="KM88XMY" className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm uppercase font-bold" /></div><div><label className="text-xs text-zinc-500">Phone *</label><input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="07XXX XXXXXX" className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm" /></div></div>
        <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-zinc-500">Car Make</label><input value={make} onChange={e=>setMake(e.target.value)} placeholder="e.g. BMW 3 Series" className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm" /></div><div><label className="text-xs text-zinc-500">Service</label><select value={service} onChange={e=>setService(e.target.value)} className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm">{services.map(s=><option key={s}>{s}</option>)}</select></div></div>
        <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-zinc-500">Drop-off Date</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm" /></div><div><label className="text-xs text-zinc-500">Time</label><select value={time} onChange={e=>setTime(e.target.value)} className="w-full mt-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm"><option>09:00</option><option>10:00</option><option>11:00</option><option>12:00</option><option>13:00</option><option>14:00</option><option>15:00</option><option>16:00</option></select></div></div>
        <button onClick={handleBooking} disabled={loading} className="w-full mt-2 py-4 bg-yellow-400 text-black rounded-xl font-bold text-lg disabled:opacity-50">{loading? "Booking..." : "Book Now - Get AG-XXXX Ref"}</button>
        <div className="flex gap-2 mt-4"><a href="/garage" className="flex-1 py-3 bg-zinc-800 rounded-xl text-center text-xs font-bold">Garage Login</a><a href="/track/AG-TEST" className="flex-1 py-3 bg-zinc-800 rounded-xl text-center text-xs font-bold">Track Booking</a></div>
        <p className="text-xs text-zinc-600 text-center mt-4">✅ Auto SMS • ✅ Invoice PDF • ✅ Cash/Card • ✅ Taxi</p>
      </div>
    </div></div>
  );
}
