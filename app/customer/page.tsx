...
export default function CustomerDashboard() {
  ...
  const fetchCars = async (email: string) => {
    ...
    if (data) {
      ...
      const carsWithMileage = await Promise.all(data.map(async (car) => {
        ...
        const oilInterval = interval?.oil_miles || 10000
  ...
  const handleAddCar = async (e: any) => {
    ...
    // Check if car already exists with someone else
    ...
    if (existingCar && existingCar.original_owner_email!== customerEmail) {
      const confirmTransfer = confirm(`Ye gadi ${reg} pehle se kisi aur ke naam hai - Kya tum new owner ho? Transfer request bheje?`)
      if (!confirmTransfer) return
      // For now simple transfer - In Phase 2 V5C upload ayega
      ...
      alert("Gadi transfer ho gayi! Ab tum owner ho - Previous owner ka private data hide rahega.")
    ...
    // Create default custom intervals - 8000 wala tum custom kar sakte ho baad mein
    await supabase.from("car_custom_intervals").insert([{
      car_reg: reg,
      owner_email: customerEmail,
      oil_miles: 8000, // Tumne kaha 8000 - Default 8000 kar diya - Customer change kar sakta hai
      brake_pad_miles: 30000,
      tyre_miles: 25000,
      mot_days_before: 30
    }])

    alert(`Gadi ${reg} add ho gayi! Ab iski history dekho.`)
    setNewReg("")
    fetchCars(customerEmail)
  }
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-4 rounded-xl shadow mb-6">
          ...
          <p className="text-xs text-gray-400 mt-2">Example: KM66XMY, AB12CDE - DVLA se auto details ayengi</p>
        </div>
        ...
        {cars.length === 0? (
          ...
          <div className="grid gap-4">
            {cars.map((car) => (
              <div key={car.id} className="bg-white p-5 rounded-xl shadow flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold tracking-wider">{car.car_reg}</h3>
                  <p className="text-sm text-gray-500">Last Mileage: {car.lastMileage} miles | Next Oil Due: {car.nextOilDue} miles ({car.oilInterval} interval)</p>
                  <p className="text-xs mt-1">
                    {car.lastMileage >= car.nextOilDue - 500 && car.lastMileage < car.nextOilDue && <span className="bg-yellow-100 text-yellow-700 px-2 py-1 rounded">⚠️ Oil due in {car.nextOilDue - car.lastMileage} miles</span>}
                    {car.lastMileage >= car.nextOilDue && <span className="bg-red-100 text-red-700 px-2 py-1 rounded">🔴 Oil OVERDUE!</span>}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Link href={`/customer/cars/${car.car_reg}/settings`} className="bg-gray-100 px-4 py-2 rounded-lg text-sm text-center">⚙️ Settings (8000)</Link>
                </div>
              </div>
            ))}
          </div>
